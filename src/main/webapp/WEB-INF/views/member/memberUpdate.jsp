  ...
  <script>
    ...
    function fCheck() {
      ...
      // 유효성검사를 위해 폼안의 내용들을 모두 변수에 담는다.
    	let nickName = myupdateform.nickName.value;
    	let name = myupdateform.name.value;
    	let email1 = myupdateform.email1.value;
    	let email2 = myupdateform.email2.value;
      let email = email1 + '@' + email2;
      let homePage = myupdateform.homePage.value;
      let tel1 = myupdateform.tel1.value;
      let tel2 = myupdateform.tel2.value;
      let tel3 = myupdateform.tel3.value;
    	let tel = tel1 + "-" + tel2 + "-" + tel3;
    	
    	// 사진 업로드 체크를 위한 준비
    	let maxSize = 1024 * 1024 * 1; 	// 업로드할 회원사진의 용량은 1MByte까지로 제한한다.
    	let fName = myupdateform.fName.value;
      ...
      if(tel2 != "" || tel3 != "") {
  		...
  		let postcode = myupdateform.postcode.value + " ";
  		let roadAddress = myupdateform.roadAddress.value + " ";
  		let detailAddress = myupdateform.detailAddress.value + " ";
  		let extraAddress = myupdateform.extraAddress.value + " ";
  		myupdateform.address.value = postcode + "/" + roadAddress + "/" + detailAddress + "/" + extraAddress + "/";
  		
  		// 전송전에 파일에 관한 사항체크...(회원사진의 내역이 비었으면 noimage를 hidden필드인 photo필드에 담아서 전송한다.)
  		if(fName.trim() == "") {
  			myupdateform.photo.value = "${vo.photo}";
				submitFlag = 1;
  		}
  		else {
    		...
    		if(nickName == '${sNickName}') {
   			...
   			if(nickCheckSw == 0) {
    			...
    			document.getElementById("nickNameBtn").focus();
	  			...
	  			myupdateform.email.value = email;
	  			myupdateform.tel.value = tel;
...
<div class="container" style="padding:30px">
  <form name="myupdateform" method="post" class="was-validated" enctype="multipart/form-data">
    <h2>회 원 정 보 수 정</h2>
    <br/>
    ...
    <div class="form-group">
      ...
      <input type="text" value="${vo.nickName}" class="form-control" id="nickName" placeholder="별명을 입력하세요." name="nickName" />
    </div>
    <div class="form-group">
      <label for="name">성명 :</label>
      <input type="text" value="${vo.name}" class="form-control" id="name" placeholder="성명을 입력하세요." name="name" readonly />
    </div>
    <div class="form-group">
    ...
    <input type="hidden" name="pastPhoto" value="${vo.photo}"/>
